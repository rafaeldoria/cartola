$.getJSON("http://localhost:8090/cartola/Usuario/lista_dados", function (retorno) {
    ...
    $('#adicionarSaldo').click(function () {
        $.getJSON("http://localhost:8090/cartola/Usuario/lista_dados", function (retorno) {
            ...
            $("#modalLabelAdicionarSaldo").text("Editar dados " + resultado[0].Nome);
            $("#nomeUsuarioSaldo").val(resultado[0].Nome);
            $("#saldoUsuario").val(resultado[0].Saldo);
            $('#modal_adiciona_saldo').modal('show');
        });
    });
});
